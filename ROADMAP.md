...
| Tests | 146 Rust tests, 130 frontend tests (`node --test`) and 49 tests for `server/`; `cargo fmt`, `cargo clippy`, `cargo test` and the frontend suite run in CI on `windows-latest` |
...
| 测试 | Rust 149 个测试、前端 130 个测试（`node --test`）、`server/` 49 个测试；CI 在 `windows-latest` 上跑 `cargo fmt`、`cargo clippy`、`cargo test` 和前端测试 |
