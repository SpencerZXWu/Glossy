...
| 服务器地址（`cloud`） | 只在 Glossy 自己的服务器下显示：翻译服务器的地址，`https://…`。构建里已经带了本项目正在运行的那个部署的地址，所以这个字段可以留空；填上它就能把应用指向你自己的部署（见 [`server/`](./server/README.md)）。字段下方会显示今天还剩多少额度——或者服务器联系不上的原因——旁边是 `重新检查` 按钮。 |
...
| `cloud` | 无需填写 | **Glossy 翻译**：由后端服务器（[`server/`](./server/README.md)）用本项目自己的账号完成翻译，应用只发送文本和一个安装 id。无需任何配置，机器上也不会有密钥，中国大陆可直接访问。每日额度按设备、按地址以及总量分别统计，设置窗口里会显示当天还剩多少；想换成自己的部署，只需要改一下服务器地址。 |
